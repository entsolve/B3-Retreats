<?php
/* =====================================================================
   B³ Retreats — Datenschutzerklaerung.

   Gebaut wie die Startseite: Vorlage + Panel-Inhalt, bei jedem Aufruf.
   Der Rumpf steht unter recht.datenschutz.body im Abschnitt
   ...
   Faellt irgendetwas aus, bleibt die Seite stehen: datenschutz.html ist
...
try {
    $html = b3_render_template(__DIR__ . '/tools/templates/datenschutz.html');
    if (trim($html) === '') {
        throw new RuntimeException('Die Vorlage hat nichts geliefert.');
    }
    echo $html;
} catch (Throwable $e) {
    error_log('B3 datenschutz: ' . $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine());

    $notfall = __DIR__ . '/datenschutz.html';
    if (is_file($notfall)) {
        readfile($notfall);
        exit;
    }
